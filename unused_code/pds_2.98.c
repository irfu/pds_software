...
int main(int argc, char *argv[])
{
  ...
  if(GetOption("-h",argc,argv,NULL))
    {
      printf("Usage: %s  [-h] [-c pds.conf] [-a pds.anomalies] [-b pds.bias] [-e pds.exclude] [-m pds.modes] [-calib] [-debug level] -mp mission_phase_abbreviation -vid volume_id -dsv data_set_version\n",argv[0]); // Display arguments
      exit(0);
    }
  ...
  if (argc>20 || argc<7) 
    {                 
      fprintf(stderr,"Usage: %s  [-h] [-c pds.conf] [-a pds.anomalies] [-b pds.bias] [-e pds.exclude] [-m pds.modes] [-calib] [-debug level] -mp mission_phase_abbreviation -vid volume_id -dsv data_set_version\n",argv[0]); // Check arguments
      exit(1);
    }
