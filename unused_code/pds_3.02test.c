...
// VERSION: 3.00
// AUTHOR: Reine Gill
// Lines 4603-4604 modified from 2.98 by aie@irfu.se, Lines 6661 & 6806-6815 edited by fj@irfu.se
// DATE: 140710
//
//====================================================================================================================
...
// Get volume id
  ...
  SetP(&cat,"START_TIME",tstr3,1);                      // Set START_TIME
...
// EDIT 6/8 2014 FKJN. VERY UGLY EDIT. Move declerations and indent when verified it works... 
//char tstr1[256];                // Temporary string
//char tstr5[256];                // Temporary string
...
//m_type *mc;
  int valid=0;                   // Valid time index
                   // Valid time of calibration data
//property_type *property1;       // Temporary property pointer
...
// START TIMES
FindP(&comm,&property1,"START_TIME",1,DNTCARE); 
TimeOfDatePDS(property1->value,&stime);     // Convert back to seconds
//  TimeOfDatePDS(tstr1,&stime);     // Convert back to seconds        

  // FIND LATEST OFFSET AND FACTOR CALIBRATION DATA STRUCTURE TO USE
  if(calib)
    {
      for(i=m_conv.n-1;i>=0;i--)
        {
          TimeOfDatePDS(m_conv.CD[i].validt,&vtime); // Convert valid time to seconds
          if(vtime<stime)
            {
              valid=i;
              break;
...
Scet2Date_2(td2,tstr3,3);

//DecodeRawTime(td2,tstr3,1); // Decode raw time to UTC
SetP(&comm,"START_TIME",tstr3,1);        // Update START_TIME in common PDS parameters
//Raw2OBT_Str(td2,pds.SCResetCounter,tstr5); // Compile OBT string with reset number of S/C clock
//SetP(&comm,"SPACECRAFT_CLOCK_START_COUNT",tstr5,1);

//STOPTIME

FindP(&comm,&property1,"STOP_TIME",1,DNTCARE); 
TimeOfDatePDS(property1->value,&stime);     // Convert back to seconds
td2=stime+td1;
Scet2Date_2(td2,tstr3,3);
//DecodeRawTime(td2,tstr3,1);
SetP(&comm,"STOP_TIME",tstr3,1);        // Update START_TIME in common PDS parameters
//Raw2OBT_Str(td2,pds.SCResetCounter,tstr5); // Compile OBT string with reset number of S/C clock
//SetP(&comm,"SPACECRAFT_CLOCK_STOP_COUNT",tstr5,1);        // Update START_TIME in common PDS parameters
}
...
int WritePTabFile(unsigned char *buff,char *fname,int data_type,int samples,int id_code,int length,sweep_type *sw_info,curr_type *curr,int param_type,int dsa16_p1,int dsa16_p2,int dop,m_type *mc,unsigned int **bias,int nbias,unsigned int **mode,int nmode,int ini_samples,int samp_plateau)
{
  char tstr1[256];                // Temporary string
  char tstr2[256];
  char tstr3[256];
      ...
      else
	{
	  if(data_type==D16)
	  ...
	  else // 20 bit
	    {
	      ...
	      if(data_type==D20T || data_type==D201T || data_type==D202T) // Truncated increase cal factor by 16
		vcalf*=16;
	    }
	}
    }
	  ...
	  if(calib)
	      ...
	      if(curr->bias_mode==DENSITY)
		{
		  if(param_type==SWEEP_PARAMS)
			...
			{
			  ccurrent=ccalf*((double)(current-(mc->CD[valid].C[vbias1][1]-mc->CD[valid].C[vbias2][2]))); // Offset and factor calibration
			  // Write time, current and calibrated voltage 
		      ...
		      if(curr->sensor==SENS_P1 || dop==1)
			{
			  ccurrent=ccalf*((double)(current-mc->CD[valid].C[vbias1][1])); // Offset and factor calibration
			  // Write time, current and calibrated voltage 
		      ...
		      if(curr->sensor==SENS_P2 || dop==2)
			{
			  ccurrent=ccalf*((double)(current-mc->CD[valid].C[vbias2][2])); // Offset and factor calibration
			  // Write time, current and calibrated voltage 
	      ...
	      if(curr->sensor==SENS_P1P2 && dop==0)
		{
		  ...
		  if(curr->bias_mode==DENSITY) 
		    fprintf(pds.stable_fd,"%s,%016.6f,%6d,%6d,%6d\r\n",tstr3,td2,current,vbias1,vbias2); // Add two voltage biasvectors
		  else
