...
// FKJN for gods sake, sort it alphanumerically first. 2/9 2014
    if((scandir(rpath, &dir, 0, alphasort)==0);
    {
      perror("opendir");
  ...
  if(calib)
      ...
      for(i=mc->n-1;i>=0;i--)
	{
	  ...
	  if(vtime<stime) 
	    {
	//FKJN TIME DEBUGGING.
            printf("FKJN message: valid = %i of %i  \n vtime(calfile)= %s, stime = %s",i,(mc->n-1),mc->CD[i].validt,tstr1);   
//	      printf("FKJN message: valid = %i ,/n in seconds: vtime= %g, stime = %g",i,vtime,stime);	
	      valid=i;
	      break;
	    }
	}
           ...
           if(data_type==D16)
            {
              if(val>=0)
              { 
               val = val +2; //this value jumps when adc is set to 0
               }
            }
