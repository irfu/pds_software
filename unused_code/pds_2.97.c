...
int  LoadBias(unsigned int ***bias_s,char *path);		// Load bias settings file
...
//----------------------------------------------------------------------------------------------------------------------------------
int WriteLabelFile(prp_type *pds,char *name);			// Write label file
...
int WritePTabFile(unsigned char *buff,char *fname,int data_type,int samples,int id_code,int length,sweep_type *sw_info,curr_type *curr,int param_type,int dsa16_p1,int dsa16_p2,int dop,m_type *m_conv,unsigned int **bias,int nbias,int ini_samples,int samp_plateau);
...
pds_type pds =
  {
    ...
    0,          // DPL Number
    0.0,        // Data Set Version, first version is 1.0
    "",         // Label revision note
    "",         // Release date       
...
int main(int argc, char *argv[])
{
  ...
  if(GetOption("-h",argc,argv,NULL))
    {
      printf("Usage: %s  [-h] [-c pds.conf] [-a pds.anomalies] [-b pds.bias] [-e pds.exclude] [-m pds.modes] [-calib] [-debug level] -mp mission_phase_abbreviation\n",argv[0]); // Display arguments
      exit(0);
    }
  ...
  if (argc>9 || argc<2) 
    {                 
      fprintf(stderr,"Usage: %s  [-h] [-c pds.conf] [-a pds.anomalies] [-b pds.bias] [-e pds.exclude] [-m pds.modes] [-calib] [-debug level] -mp mission_phase_abbreviation\n",argv[0]); // Check arguments
      exit(1);
    }
  ...
  // Will not work, since required parameter value -mp is missing!!
  // ----------------------------------------------------------------------------------------------------
  ...
  if((status=LoadConfig1(&pds))<0) 
    {
      fprintf(stderr,"Mangled configuration file: %d\n",status); // Check arguments
      exit(1);
    }
  ...
  if(LoadTimeCorr(&pds,&tcp)<0)         // Load the time correlation packets, once and for all!
    {
      ...
      fprintf(stderr,"Warning: No time corellation packets found\n"); 
  ...
  SetP(&cat,"DATA_SET_ID",mp.data_set_id,1);            // Set DATA SET ID 
  SetP(&cat,"DATA_SET_NAME",mp.data_set_name,1);        // Set DATA SET NAME
  SetP(&cat,"PUBLICATION_DATE",pds.ReleaseDate,1);      // Set PUBLICATION date of data set 
  WriteLabelFile(&cat,tstr1);                           // Write back label file with new info
  ...
  if(calib)
   ...
   SetP(&cc_lbl,"DATA_SET_ID",mp.data_set_id,1);        // Set DATA SET ID in calibration file
   SetP(&cc_lbl,"DATA_SET_NAME",mp.data_set_name,1);    // Set DATA SET NAME in calibration file     
   WriteLabelFile(&cc_lbl,pds.cpathc);                  // Write back label file with new info
   ...
   SetP(&fc_lbl,"DATA_SET_ID",mp.data_set_id,1);        // Set DATA SET ID in calibration file
   SetP(&fc_lbl,"DATA_SET_NAME",mp.data_set_name,1);    // Set DATA SET NAME in calibration file     
   WriteLabelFile(&fc_lbl,pds.cpathf);                  // Write back label file with new info
   ...
   SetP(&ic_lbl,"DATA_SET_ID",mp.data_set_id,1);        // Set DATA SET ID in calibration file
   SetP(&ic_lbl,"DATA_SET_NAME",mp.data_set_name,1);    // Set DATA SET NAME in calibration file     
   WriteLabelFile(&ic_lbl,pds.cpathi);                  // Write back label file with new info
   ...
   SetP(&tmp_lbl,"DATA_SET_ID",mp.data_set_id,1);       // Set DATA SET ID in calibration file
   SetP(&tmp_lbl,"PUBLICATION_DATE",pds.ReleaseDate,1); // Set publication date of dataset
   WriteLabelFile(&tmp_lbl,pds.cpathdfp1);              // Write back label file with new info
   ...
   SetP(&tmp_lbl,"DATA_SET_ID",mp.data_set_id,1);       // Set DATA SET ID in calibration file
   SetP(&tmp_lbl,"PUBLICATION_DATE",pds.ReleaseDate,1); // Set publication date of dataset
   WriteLabelFile(&tmp_lbl,pds.cpathdfp2);              // Write back label file with new info
   
   FreePrp(&tmp_lbl);                                   // Initialize property value pair list
   status=+ReadLabelFile(&tmp_lbl,pds.cpathefp1);       // Read e-field frequency response calibration file probe 1
   SetP(&tmp_lbl,"DATA_SET_ID",mp.data_set_id,1);       // Set DATA SET ID in calibration file
   SetP(&tmp_lbl,"PUBLICATION_DATE",pds.ReleaseDate,1); // Set publication date of dataset
   WriteLabelFile(&tmp_lbl,pds.cpathefp1);              // Write back label file with new info
   
   FreePrp(&tmp_lbl);                                   // Initialize property value pair list
   status=+ReadLabelFile(&tmp_lbl,pds.cpathefp2);       // Read e-field frequency response calibration file probe 2
   SetP(&tmp_lbl,"DATA_SET_ID",mp.data_set_id,1);       // Set DATA SET ID in calibration file
   SetP(&tmp_lbl,"PUBLICATION_DATE",pds.ReleaseDate,1); // Set publication date of dataset
   WriteLabelFile(&tmp_lbl,pds.cpathefp2);              // Write back label file with new info
   
   status=+GetMCFiles(pds.cpathd,pds.cpathm,&m_conv);   // Get measurment calibration files
...
{   
  ...
  while(1)
    {  
      ...
      sprintf(tstr2,"\"%s, %s\"",tstr1,pds.LabelRevNote);	// Assemble label revison note
      SetP(&hkl,"LABEL_REVISION_NOTE",tstr2,1);			// Set LABEL Revision note
...
{ 
  ...
  int macro_status=0;             // Indicates if we found a matching macro desription for the macro id
 

  unsigned int meas_seq = 0;      // Measurment sequence
...
// Structure with current settings for various parameters
  ...
  if((nbias=LoadBias(&bias,pds.bpath))<0) 
    {
  ...
  while(1) 
	...
	case S01_GET_MAINH: 
	  ...
	  macro_status=1;   // Asume initially that no macro descripition is found
	  ...
	  if(buff[0]==M_HEAD) 
		...
		main_h_sum=M_HEAD;   // Nope force main_h_sum to header byte
	      ...
	      in_sync=1;             // Assume we are in sync
	      byte_sum=0;
	      meas_seq=0;            // Set number of measurment sequences to none
	...
	case S04_GET_ID_CODE:
	  ...
	  if(buff[0]==0xCC) // Found Sub Header
	    { 
	      ...
	      // At this point:
	      // If anomally found in pds.anomalies
	      ...
	      if(macro_priority && !macro_status) 
		{
		  if((val=FindIDCode(&macros[mb][ma],meas_seq+1))<0)
		    ...
		    {
		      CPrintf("Anomally correction overrides ID Code %d with %d for sequence %d in current data set\n",id_code,val,meas_seq+1);
		      id_code=val;
		    }
		}
		...
		case GENERIC_ID_P1:
		  ...
		  if(buff[0]==0xCC && buff[1]==GENERIC_ID_P2)
		  ...
		  else
		    {
		      ...
		      meas_seq++; // Increase number of measurment sequences
	...
	case S08_DECODE_PARAMS:
	  ...
	  if(param_type==SWEEP_PARAMS) // DECODE SWEEP PARAMETERS
		...
		strcpy(sw_info.p2,"NO");
	      ...
	      InsertTopQV(&dict,"ROSETTA:LAP_SWEEP_START_BIAS",sw_info.start_bias); 
	      InsertTopQV(&dict,"ROSETTA:LAP_SWEEP_STEP_HEIGHT",sw_info.height); 
	      InsertTopQV(&dict,"ROSETTA:LAP_SWEEP_STEPS",sw_info.steps); 
	      InsertTopQV(&dict,"ROSETTA:LAP_SWEEP_PLATEAU_DURATION",sw_info.plateau_dur);
	      InsertTopQ(&dict,"ROSETTA:LAP_SWEEP_RESOLUTION",sw_info.resolution); 
	      InsertTopQ(&dict,"ROSETTA:LAP_SWEEP_FORMAT",sw_info.format); 
	      ...
	      for(i=0;i<dict.no_prop;i++) 
		{
		  ...
		  if(FindP(&macros[mb][ma],&property2,property1->name,1,UNCHECKED)>0) // Look for unchecked ones 
		      ...
		      if(ComparePrp(property1,property2)) // Compare properties
			{
		      ...
		      // comparison in the current measurment cycle.
	...
	case S14_RESOLVE_MACRO_PARAMETERS: 
	  ...
	  // Finger printing for the right macro description, this is a bit dirty. 
	  ...
	  // a macro that return a macro id. But we can also run
	  // the instrument from the old prom macros that dosn't
	  ...
	  if(param_type==ADC20_PARAMS) // Test if 20 Bit ADC:s are involved!
	    {
	      switch(curr.sensor)
		{
		case SENS_P1:	
	  ...
	  if(!macro_status) // If a macro description exists
	    {
	      //Find subheader for measurment sequence meas_seq
	      if(FindP(&macros[mb][ma],&property1,"ROSETTA:LAP_SET_SUBHEADER",meas_seq,DNTCARE)>0)
		{
		  ...
		  if(curr.sensor==SENS_P1 || curr.sensor==SENS_P1P2)
		      ...
		      if(param_type!=SWEEP_PARAMS)
			{
			  if(FindB(&macros[mb][ma],&property1,&property2,"ROSETTA:LAP_P1_BIAS_MODE",DNTCARE)>0)
			    {
			      if(curr.bias_mode==E_FIELD) // Curent mode according to ID is E-FIELD
				{
			      ...
			      if(curr.bias_mode==DENSITY)// Curent mode according to ID is DENSITY
				{
				  // Is it so according to macro description , or macro description has high priority
		  ...
		  if(FindP(&macros[mb][ma],&property2,"ROSETTA:LAP_TM_RATE",1,DNTCARE)>0) 
		    {
		      InsertTopK(&dict,property2->name,property2->value); // Set it in dictionary
	      ...
	      if(param_type==NO_PARAMS)
		{
	      ...
	      CPrintf("    OBT time start of measurment cycle: %s \n",tstr5);
...
//"DATA/CALIBRATED/2003/JAN/D16/ ","RPCLAP030116_01NS_REB20BS.LBL","2004-04-15T12:50:22","2003-01-16T08:20:59.207","2003-01-16T08:20:59.316","MCID0x212"<CR><LF>
		...
		{ 
		  ...
		  if(WritePLBL_File(pds.spaths,lbl_fname,&curr,samples,id_code,0,ini_samples,param_type)>=0)
		    {
		    WritePTabFile(buff,tab_fname,data_type,samples,id_code,length,&sw_info,&curr,param_type,dsa16_p1,dsa16_p2,0,&m_conv,bias,nbias,ini_samples,samp_plateau);
	      ...
	      else // Split interleaved 20 Bit data into two pairs of label and tab files
		{
		  ...
		  if(WritePLBL_File(pds.spaths,lbl_fname,&curr,samples,id_code,1,ini_samples,param_type)>=0)
		    {
		      WritePTabFile(buff,tab_fname,data_type,samples,id_code,length,&sw_info,&curr,param_type,dsa16_p1,dsa16_p2,1,&m_conv,bias,nbias,ini_samples,samp_plateau);  
		  ...
		  if(WritePLBL_File(pds.spaths,lbl_fname,&curr,samples,id_code,2,ini_samples,param_type)>=0)
		    {
		      WritePTabFile(buff,tab_fname,data_type,samples,id_code,length,&sw_info,&curr,param_type,dsa16_p1,dsa16_p2,2,&m_conv,bias,nbias,ini_samples,samp_plateau);
	      ...
	      ClearDictPDS(&dict);   // Clear dictionary PDS LAP parameters, common parameters are not cleared until a new measurment cycle beginns
	    }
	  else
	    {
...
{ 
  ...
  sprintf(p->tpath,"%s%s",p->apathdds,"/AUX/TIME_CORR_COFF"); // Setup path to time correlation packets
...
int  LoadBias(unsigned int ***bias_s,char *path) // Load bias settings
{
  FILE *fd;
  char line[256]; // Line buffer
...
int  LoadTimeCorr(pds_type *pds,tc_type *tcp)
{
 ...
 if((dir = opendir(dir_n))==NULL) // Open time correlation packet directory
  ...
  while(dentry!=NULL) // Do a linear search through all files
	...
	{
	  Separate(dentry->d_name,l_tok,r_tok,'Z',1);
	  TimeOfDatePDS(&r_tok[1],&etime);
	  if(etime>otime)
	    {
	      otime=etime;
...
// Write label file
int WriteLabelFile(prp_type *lb_data,char *name)
{
  FILE *fd;
...
int WritePTabFile(unsigned char *buff,char *fname,int data_type,int samples,int id_code,int length,sweep_type *sw_info,curr_type *curr,int param_type,int dsa16_p1,int dsa16_p2,int dop,m_type *mc,unsigned int **bias,int nbias,int ini_samples,int samp_plateau)
{
  char tstr1[256];                // Temporary string
  char tstr2[256];
  char tstr3[256];
  ...
  double vcalf;                   // Calibration factor voltage
  double ccalf;                   // Calibration factor current
  ...
  // FIND LATEST OFFSET AND FACTOR CALIBRATION DATA STRUCTURE TO USE
  if(calib)
    {
      ...
      for(i=mc->n-1;i>=0;i--)
	{
      ...
      // Find the correct calibration factors depending on E-FIELD/DENSITY, GAIN, 20Bit/16Bit ADC
        
      if(curr->bias_mode==DENSITY) // Check if current data origins from a density mode measurment
	{
	  if(data_type==D16)
	    {
	      // We have the same calibration factor for both P1 and P2 for now!
	      if(curr->sensor==SENS_P1 || curr->sensor==SENS_P1P2 || dop==1) 
		{
		  if(!strncmp(curr->gain1,"\"GAIN 1\"",8))
		    {
		...
		{
		   if(!strncmp(curr->gain2,"\"GAIN 1\"",8))
		     {
	      ...
	      if(curr->sensor==SENS_P1 || dop==1)
		{
		  ...
		  if(!strncmp(curr->gain1,"\"GAIN 1\"",6))
		    {
		  ...
		  if(data_type==D20T || data_type==D201T) // If 20 Bit data is truncated 4 bits, increase calibration factor by 16
		    ccalf*=16;
		}
	      
	      if(curr->sensor==SENS_P2 || dop==2)
		{
		  if(!strncmp(curr->gain2,"\"GAIN 1\"",6))
		    {
		  ...
		  if(data_type==D20T || data_type==D202T) // If 20 Bit data is truncated 4 bits, increase calibration factor by 16
		    ccalf*=16;
		}
	      // Other alternative than above shouldn't be possible..if so keep default ccalf
	    }
  ...
  if((pds.stable_fd=fopen(tstr2,"w"))==NULL)                       // Open table file
	...
	{ 
	  ...
	  if(nbias>0 && bias!=NULL)
	    {
	      ...
	      // First time set old_time to current time..only once!
	      if(curr->old_time<EPS && curr->old_time>-EPS) 
		curr->old_time=utime;


	      for(l=0;l<nbias;l++) // Go through all extra bias settings
		{
		  if(curr->old_time<bias[l][0] && bias[l][0]<=utime) // Any bias setting done since last one
		    {
		      curr->vbias1= ((bias[l][1] & 0xff00)>>8); // Override macro present voltage bias p1
		      curr->vbias2=  (bias[l][1] & 0xff);       // Override macro present voltage bias p2
	  ...
	  // Reset biases, extra bias setting might have been done
	  if(curr->sensor==SENS_P1 || dop==1)
	    {
	      vbias=curr->vbias1;
	      ibias=curr->ibias1;
	  ...
	  if(curr->bias_mode==DENSITY) // Check if current data origins from a density mode measurment
		...
		voltage=vbias; // Set FIX Density bias in TM units
	    }
	  ...
	  if(calib)
	      ...
	      if(curr->bias_mode==DENSITY)
		{
		  if(param_type==SWEEP_PARAMS)
		      ...
		      if(strcmp(sw_info->resolution,"FINE")) // If it's not a fine sweep...
			{
			  if(curr->sensor==SENS_P1)
			      {
				ccurrent=ccalf*((double)(current-mc->CD[valid].C[voltage][1])); // Offset and factor calibration
			  ...
			  if(curr->sensor==SENS_P2)
			      {
				ccurrent=ccalf*((double)(current-mc->CD[valid].C[voltage][2])); // Offset and factor calibration
				// Write time, current and calibrated voltage 
			  ...
			  if(curr->sensor==SENS_P1)
			    {
			      ...
			      ccurrent=ccalf*((double)(current-mc->CD[valid].C[voltage][1])); 
			      // Write time, current and calibrated voltage 
			      fprintf(pds.stable_fd,"%s,%016.6f,%14.7e,%14.7e\r\n",tstr3,td2,ccurrent,f_conv.C[sw_info->p1_fine_offs*voltage][2]); 
			    }
			  ...
			  if(curr->sensor==SENS_P2)
			    {
			      ...
			      ccurrent=ccalf*((double)(current-mc->CD[valid].C[voltage][2])); 
			      // Write time, current and calibrated voltage 
			      fprintf(pds.stable_fd,"%s,%016.6f,%14.7e,%14.7e\r\n",tstr3,td2,ccurrent,f_conv.C[sw_info->p1_fine_offs*voltage][3]); 
			    }
 			}
		    }
		  else // Not a sweep but density mode
		    {

		      if(curr->sensor==SENS_P1P2 && dop==0)
			{
			  ccurrent=ccalf*((double)(current-(mc->CD[valid].C[curr->vbias1][1]-mc->CD[valid].C[curr->vbias2][2]))); // Offset and factor calibration
			  // Write time, current and calibrated voltage 
			  fprintf(pds.stable_fd,"%s,%016.6f,%14.7e,%14.7e,%14.7e\r\n",tstr3,td2,ccurrent,v_conv.C[curr->vbias1][1],v_conv.C[curr->vbias2][2]); 
			}
		      
		      if(curr->sensor==SENS_P1 || dop==1)
			{
			  ccurrent=ccalf*((double)(current-mc->CD[valid].C[curr->vbias1][1])); // Offset and factor calibration
			  // Write time, current and calibrated voltage 
			  fprintf(pds.stable_fd,"%s,%016.6f,%14.7e,%14.7e\r\n",tstr3,td2,ccurrent,v_conv.C[curr->vbias1][1]); 
			}

		      if(curr->sensor==SENS_P2 || dop==2)
			{
			  ccurrent=ccalf*((double)(current-mc->CD[valid].C[curr->vbias2][2])); // Offset and factor calibration
			  // Write time, current and calibrated voltage 
			  fprintf(pds.stable_fd,"%s,%016.6f,%14.7e,%14.7e\r\n",tstr3,td2,ccurrent,v_conv.C[curr->vbias2][2]);
			}
		    }
	      ...
	      else // Assume bias mode is E_FIELD no other possible
		{
		  ...
		  cvoltage=vcalf*((double)voltage); // Voltage factor calibration

		  if(curr->sensor==SENS_P1P2 && dop==0)
		    fprintf(pds.stable_fd,"%s,%016.6f,%14.7e,%14.7e,%14.7e\r\n",tstr3,td2,i_conv.C[curr->ibias1][1],i_conv.C[curr->ibias2][2],cvoltage); // Write time, calibrated current and voltage 
		  
		  if(curr->sensor==SENS_P1 || dop==1)
		    fprintf(pds.stable_fd,"%s,%016.6f,%14.7e,%14.7e\r\n",tstr3,td2,i_conv.C[curr->ibias1][1],cvoltage); // Write time, calibrated current and voltage 
		  
		  if(curr->sensor==SENS_P2 || dop==2)
		    fprintf(pds.stable_fd,"%s,%016.6f,%14.7e,%14.7e\r\n",tstr3,td2,i_conv.C[curr->ibias2][2],cvoltage); // Write time, calibrated current and voltage  
		}
	    }
	  else // RAW DATA
	    {
	      if(curr->sensor==SENS_P1P2 && dop==0)
		{
		  ...
		  if(curr->bias_mode==DENSITY) 
		    fprintf(pds.stable_fd,"%s,%016.6f,%6d,%6d,%6d\r\n",tstr3,td2,current,curr->vbias1,curr->vbias2); // Add two voltage biasvectors
		  else
		    fprintf(pds.stable_fd,"%s,%016.6f,%6d,%6d,%6d\r\n",tstr3,td2,curr->ibias1,curr->ibias2,voltage); // Add two current bias vectors
		}
	      else
...
int WritePLBL_File(char *path,char *fname,curr_type *curr,int samples,int id_code,int dop,int ini_samples,int param_type)
{
  ...
  if((pds.slabel_fd=fopen(fullname,"w"))==NULL)
	...
	SetP(&comm,"RECORD_BYTES",tstr2,1); // Set number of bytes in a column of a record
      ...
      if(param_type==SWEEP_PARAMS)
	InsertTopQV(&dict,"ROSETTA:LAP_INITIAL_SWEEP_SMPLS",ini_samples); 
      ...
      fprintf(pds.slabel_fd,"DESCRIPTION = \"SPACE CRAFT ONBOARD TIME SSSSSSSSS.FFFFFF (TRUE DECIMALPOINT)\"\r\n");
      fprintf(pds.slabel_fd,"END_OBJECT = COLUMN\r\n");
  ...
  if(FreePrp(p)>=0) // Free old stuff
    {
      ...
      Append(p,"PRODUCER_ID","RG");
      Append(p,"PRODUCER_FULL_NAME","\"REINE GILL\"");
  ...
  if(p!=NULL) 
    {
      ...
      Append(p,"DESCRIPTION","\"SPACE CRAFT ONBOARD TIME SSSSSSSSS.FFFFFF (TRUE DECIMALPOINT)\"");
      Append(p,"END_OBJECT","COLUMN");
      Append(p,"OBJECT","COLUMN");		  
      Append(p,"NAME","PMAC");  
  ...
  if(FreePrp(p)>=0) // Free old stuff
    {
      Append(p,"PDS_VERSION_ID","PDS3"); 
      if(calib)
	Append(p,"RECORD_BYTES","156");
      else
	Append(p,"RECORD_BYTES","157");
      ...
      Append(p,"ROWS","XX");
      if(calib)
	Append(p,"ROW_BYTES","156");
      else
	Append(p,"ROW_BYTES","157");
      Append(p,"COLUMNS","6");
	...
	Append(p,"START_BYTE","144");
      ...
      Append(p,"DATA_TYPE","CHARACTER");
      if(calib)
	Append(p,"START_BYTE","150");
      else
	Append(p,"START_BYTE","151");
...
// Find id code in macro overriding id code in data if anomally correction is
// set for the current SCET, on error -1 is returned.
// If not the id_code for the measurment sequence n in the current data set
// is returned.	
...
// Display current state of statemachine
...
int TrimQN(char *str)
{
  ...
  for(i=0;i<len;i++) // First make all newlines and carridge returns to whites
    if(str[i]=='\n' || str[i]=='\r') str[i]=' ';
