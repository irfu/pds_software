...
//  double ADC_offset =0.0;	// due to ADC errors around 0 (for 16bit data, possibly 20bit data also), we need to correct small offset
  double ocalf = 1.0;		//FKJN: offset for 20 bit data needs to be taken into account  
  ...
  bool D20_MA_on;		//D20 Moving average bug boolean
  ...
  if((pds.stable_fd=fopen(tstr2,"w"))==NULL)                       // Open table file
	...
	{ 
	  ...
	  switch(data_type)
		...
		j+=2;
      		 //EDIT FKJN 12/2 2015. Error in LAP MA SOFTWARE FOR 20 BIT DATA, MAKING LAST 4 BITS GARBAGE
	      if(D20_MA_on)	
                val=buff[j]<<12 | buff[j+1]<<4;
	      else
...
If a biascommand is issued on forbidden macros, the bias will change for maximum one Macro Loop(see Meds), but pds will not know when it changes back
 */  
// FKJN 2014-10-31 added macro 515 & 807
              if(extra_bias_setting)
                {
			FindP(&comm,&property1,"INSTRUMENT_MODE_ID",1,DNTCARE);	 //tstr4 is now macro id on form "MCID0X%04x" we need the 4numerals
			macro_id=atoi(strndup(property1->value+6,4)); //macroid is now tstr4 stripped of MCID0X and converted to number
		  ...
		  if(param_type==SWEEP_PARAMS)
		      ...
		      if(strcmp(sw_info->resolution,"FINE")) // If it's not a fine sweep...
			{
			  if(curr->sensor==SENS_P1)
			      {
				//  FKJN  2014-09-02
				// if 20bit datal, ocalf = 16, otherwise 1.
			...
			{
			  ccurrent=ccalf*((double)(current-ocalf*(mc->CD[valid].C[vbias1][1]-mc->CD[valid].C[vbias2][2]))); // Offset and factor calibration
			  // Write time, current and calibrated voltage 
		      ...
		      if(curr->sensor==SENS_P1 || dop==1)
			{
			  ccurrent=ccalf*((double)(current-ocalf*mc->CD[valid].C[vbias1][1])); // Offset and factor calibration
			  // Write time, current and calibrated voltage 
		      ...
		      if(curr->sensor==SENS_P2 || dop==2)
			{
			  ccurrent=ccalf*((double)(current-ocalf*mc->CD[valid].C[vbias2][2])); // Offset and factor calibration
			  // Write time, current and calibrated voltage 
