...
//            printf("FKJN debug: valid = %i of %i  \n vtime(calfile)= %s, stime = %s \n",i,(mc->n-1),mc->CD[i].validt,tstr1);   
      ...
      if(curr->bias_mode==DENSITY) // Check if current data origins from a density mode measurement
	{
	  ...
	  if(data_type==D16)
		...
		{
                  // Seĺect ADC20 calibration "delta" constant. This should ideally depend on high/low gain but we use the high-gain value for now. /Erik P G Johansson 2015-06-03.
